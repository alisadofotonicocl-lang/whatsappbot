...
app.get("/", async (req, res) => {
  const banner = hasAPIKey()
    ...
    : `<div style="background:#fff3cd;color:#664d03;padding:12px;border-radius:8px;border:1px solid #ffe69c;margin-bottom:16px">
         <b>Falta OPENAI_API_KEY</b>: agrégala en Railway → Variables y redeploy. El QR igualmente puede mostrarse.
       </div>`;
  ...
  <style>
    ...
    <div class="card">
      ...
      <p style="opacity:.7">La <b>OPENAI_API_KEY</b> solo se agrega en Railway → Variables (nunca en el repo).</p>
    </div>
  </body></html>`);
});
...
async function askOpenAI(messages) {
  if (!hasAPIKey()) {
    return "⚠️ Falta configurar la OPENAI_API_KEY en Railway → Variables. Cuando la agregues y redeployes, podré responder con IA.";
  }
...
// ⚠️ IMPORTANTE: el llamado va AL FINAL del archivo, después de definir todo.
